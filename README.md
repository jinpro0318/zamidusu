...
## 소셜 로그인(Google OAuth) 설정 — Supabase

> 로그인 provider는 **Google**입니다. (Kakao 로그인은 비활성 이슈로 제외 —
> 카카오톡 **공유**는 `NEXT_PUBLIC_KAKAO_JS_KEY`(JS키)로 동작하는 **별개 기능**이라 그대로 유지.)
>
> **증상(잘못 꺼져 있을 때)**: 「Google로 계속하기」 클릭 시
...
> **원인**: Supabase Auth에서 Google provider가 꺼져 있음 (코드 문제 아님).

**1) Google Cloud Console** (https://console.cloud.google.com → APIs & Services → Credentials)
- **OAuth 2.0 Client ID** 생성 (Application type: Web application).
...
- **Enable** 토글 ON.
- *Client ID* / *Client Secret* = 위에서 발급받은 값.
...
- 호출부: `components/ziwei/sheets/LoginGate.tsx`, `components/ziwei/screens/Login.tsx`
  → `signInWithOAuth({ provider:'google', options:{ redirectTo: \`\${origin}/auth/callback?redirectTo=\${encodeURIComponent(...)}\` }})`
