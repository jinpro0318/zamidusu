...
## 카카오 로그인(OAuth) 설정 — Supabase

> **증상**: 로그인/공유 게이트에서 「카카오로 시작하기」 클릭 시
> `…supabase.co/auth/v1/authorize?provider=kakao` 로 이동 →
> `{"error_code":"validation_failed","msg":"Unsupported provider: provider is not enabled"}`.
> **원인**: Supabase Auth에서 Kakao provider가 꺼져 있음 (코드 문제 아님).

> ⚠️ **키 구분 주의**: 카카오톡 **공유**는 `NEXT_PUBLIC_KAKAO_JS_KEY`(JavaScript 키),
> 카카오 **로그인(OAuth)**은 아래 **REST API 키 + Client Secret**으로 서로 다릅니다.

**1) 카카오 개발자 콘솔** (https://developers.kakao.com → 내 애플리케이션)
- **카카오 로그인 → 활성화** ON.
- **Redirect URI** 등록: `https://<project-ref>.supabase.co/auth/v1/callback`
  (← Supabase가 OAuth 코드를 받는 주소. 우리 앱의 `/auth/callback`이 아님에 주의.)
- **동의항목**: 닉네임 / 이메일(이메일 수집은 비즈앱 전환이 필요할 수 있음).
- **앱 키 → REST API 키** = Supabase의 *Client ID*.
- **보안 → Client Secret** 생성 = Supabase의 *Client Secret*.

**2) Supabase 대시보드 → Authentication → Providers → Kakao**
- **Enable** 토글 ON.
- *Client ID* = 카카오 REST API 키, *Client Secret* = 카카오 Client Secret.

**3) Supabase → Authentication → URL Configuration → Redirect URLs** 허용목록 추가
- `https://zamidusu.vercel.app/auth/callback`
- `http://localhost:3000/auth/callback`
- (Site URL도 배포 URL로 설정)

**코드 측 (이미 정상)**
- 호출부: `components/ziwei/sheets/LoginGate.tsx`, `components/ziwei/screens/Login.tsx`
  → `signInWithOAuth({ provider:'kakao', options:{ redirectTo: \`\${origin}/auth/callback?redirectTo=\${encodeURIComponent(...)}\` }})`
- 콜백: `app/auth/callback/route.ts` → `exchangeCodeForSession` → 게스트 데이터 인계 → 내부 경로로만 리다이렉트(오픈 리다이렉트 가드 적용).
