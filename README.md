...
2. Vercel Marketplace에서 **Neon Postgres** 추가 → `DATABASE_URL` 자동 주입
3. `vercel env add` 또는 대시보드에서 모든 시크릿 등록:
   - `NEXTAUTH_SECRET`, `KAKAO_*`, `AI_GATEWAY_API_KEY`, `TOSS_*`, `SHARE_TOKEN_SECRET`, `CRON_SECRET`, `NEXT_PUBLIC_KAKAO_JS_KEY`
4. `prisma/schema.prisma`의 `provider`를 `"postgresql"`로 전환
5. push → 자동 배포

`vercel.json`에 Cron(`/api/cron/billing` 매일 03:00) 등록되어 있습니다.
