...
> **v2.0 주요 변경 (이번 마일스톤)**
> - 💳 **수익화 실구현**: chart 1개 = "깊은 풀이"를 **890원 무통장입금** 단건 결제로 영구 잠금해제 (관리자 입금확인 → PAID). `Purchase` 모델 + 결제 안내 모달.
...
| **깊은 풀이(전체풀이)** | **890원** | **결제(PAID)** — chart 1개 영구 잠금해제 | 단건 수익 |
...
- **대화형 상담 톤** ⭐(v5): 옆에서 상담하듯 흐르는 한 편의 글 — 이름 있으면 "○○ 님" 호칭, 도입 공감(나이/인생 단계+궁 주제) → 마음 읽기 질문 1개 → 명반 근거 따뜻한 본문 → 응원 마무리. 대괄호 머리글·`[[..]]` 용어 마킹·코드 토큰을 출력하지 않음(한자는 "한글(한자) — 풀이"로 본문에 흡수). 별 한자/밝기는 `star-names.ts` 단일 소스 주입값만 사용. 공통 규칙은 `CONSULT_STYLE` 상수.
- **응답 캐싱·가속** ⭐: 초기 풀이는 `PalaceReading`에 캐시(버전 키 `modelVersion`+`promptVersion`) → 재방문 시 LLM 호출 없이 동일 스트림으로 즉시 반환. `PROMPT_VERSION` 인상(v5) 시 옛 캐시 자동 무효화·재생성. 초기 풀이엔 `thinkingConfig.thinkingBudget=0` + `maxTokens`로 첫 토큰(TTFB) 단축. 후속 자유질문은 기본값 유지.
...
- **출력 형식**: 궁별 상세풀이와 동일한 **대화형 상담 톤**(v5) — 분야별 대괄호 섹션·`[[..]]` 마킹 없이, 성격·일·연애·재물·마음/건강을 하나의 흐르는 상담 글로 엮음(`CONSULT_STYLE` 공통).
...
| 결제 성향 | 소액(890원 깊은풀이) 충동 결제 가능 · 무통장입금/카카오페이 선호 |
...
- 890원 소액 충동 결제 성향으로 MVP 수익화 검증에 가장 적합
