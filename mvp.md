...
> **한 줄 요약**: "사주 다음은 자미두수." — 회원가입 없이 1초 만에 만드는 내 명반, AI가 한자 근거로 핵심을 짚어주는 12궁, 인스타로 공유하는 우주.

기획서 명세를 **Next.js 16 풀스택**으로 구현했고, 경쟁(포스텔·헬로우봇)과 차별화되는 **회원가입 없는 게스트 경험 + 무료=핵심 포인트·유료=심화 상담 2단 AI + 자미두수 단일 특화 + 모던 UX**를 결합했다.
...
## 3. AI 풀이 — 무료=핵심 포인트 / 유료=심화 상담 (2단 구조, 핵심 차별점)

토큰·비용을 아끼고 무료/유료를 명확히 가르기 위해 AI 풀이를 **두 단계**로 나뀐다. 규칙은 `lib/ai/prompt-builder.ts`가 단일 출처이며, `PROMPT_VERSION`을 올리면 옷 캐시가 자동 무효화된다(현재 **v7**).

- **무료 — 궁별 핵심 포인트(`BRIEF_STYLE`)**: 사주아이처럼 **한자(漢字) 근거로 핵심만** 짚는다. "한글(한자) 근거 → 성향·흐름" 형식의 짧은 포인트 4~6개 + 마지막에 생활 개운 팁 1개. 대화체·장문 없이 간결하게(무료 per-palace `maxTokens` 700).
- **유료 — 심화 상담(`CONSULT_STYLE`)**: 12궁 전체 풀이·대운 타임라인·월간·궁합은 **옆에서 상담하듯 흘르는 한 편의 글**(아래 4박자)로 더 깊게 답한다. 무료의 "핵심 포인트"를 "왜 그런지 + 어떻게 살릴지"로 확장하는 것이 유료의 차별점.

**유료 심화 상담 구조(4박자, 자연스러운 문단 흐름):**
...
| 기능 | 경로 | 접근 | 설명 |
|---|---|---|---|
| 무료 12궁 요약 | `/chart/[id]` | 누구나(게스트) | 정적 요약 + 명반 차트 + 추천 칩 |
| 궁별 핵심 포인트(AI·무료) | `/chart/[id]/palace/[key]` | 누구나(게스트) | 한자 근거로 핵심만(`BRIEF_STYLE`) + 심화 질문은 유료 안내 |
| 12궁 전체 풀이(AI·유료) | `/chart/[id]/deep` | 유료 | 12궁 종합 심화 상담 + 추천 칩 + 이어 묻기 |
| 대운·세운 타임라인(유료) | `/chart/[id]/timeline` | 유료 | Recharts 점수 그래프 + 연령대별 AI 흐름 분석 |
| 월간 운세(유료) | `/chart/[id]/monthly` | 유료 | 이번 달 흐름(달마다 새로 캐시) |
...
수익은 명반(chart) 단위 **프리미엄 풀이 잠금해제**. 핵심 차별 축은 **"보여주기(무료) vs 물어보기(유료)"** — 무료는 한자 근거로 핵심을 읽어주는 **읽기 전용 핵심 포인트**, 유료는 심화 질문에 답하는 **인터랙티브 종합 상담**.

| 콘텐츠 | 가격 | 접근 |
|---|---|---|
| 무료 12궁 요약 | 0원 | 누구나(게스트) |
...
> AI 결과 캐시는 **버전 키 설계**(`modelVersion`+`promptVersion`) — 프롬프트/모델 개선 시 `PROMPT_VERSION` 한 단계 올리면 옷 캐시가 자동 무효화·재생성된다(현재 **v7**).
...
- 모드별 시스템 프롬프트: 궁별 무료(`buildSystemPrompt`→`BRIEF_STYLE` 핵심 포인트) / 전체풀이(`buildDeepReadingPrompt`) / 대운 흐름(`buildTimelinePrompt`) / 월간(`buildMonthlyPrompt`) / 궁합(`buildCompatPrompt`). 유료 모드는 `CONSULT_STYLE` 심화 상담(§3)
...
- 게스트(쿠키 신원) 기반. 무료 per-palace는 `BRIEF_STYLE`+`maxTokens` 700으로 토큰 절감, 유료 심화는 `CONSULT_STYLE`로 상세하게
